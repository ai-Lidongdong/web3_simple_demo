...
contract NFTMarketplace is ReentrancyGuard {
    ...
    // 事件：记录订单生命周期（供链下服务监听）
    event OrderCreated(uint256 indexed orderId, address indexed seller, address nftContract, uint256 tokenId, uint256 price);
    event OrderCancelled(uint256 indexed orderId);
    event OrderExecuted(uint256 indexed orderId, address indexed buyer, address indexed seller, uint256 price);
    ...
    function createOrder(
        ...
        emit OrderCreated(orderId, msg.sender, nftContract, tokenId, price);
    }
    ...
    function cancelOrder(uint256 orderId) external nonReentrant {
        ...
        // 检查：订单存在且有效
        require(order.isActive, "订单无效或已取消");
        // 检查：调用者必须是卖家
        require(order.seller == msg.sender, "无权取消订单");
     ...
     */
    function buyNFT(uint256 orderId) external payable nonReentrant {
        Order storage order = orders[orderId];
        ...
        if (paymentToken == address(0)) {
            ...
            // 转账给卖家
            require(token.transferFrom(msg.sender, seller, sellerAmount), "卖家代币转账失败");
            // 转账手续费给平台
            require(token.transferFrom(msg.sender, admin, platformFeeAmount), "平台手续费转账失败");
        }
