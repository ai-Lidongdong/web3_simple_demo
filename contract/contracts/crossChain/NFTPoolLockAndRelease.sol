...
contract Messenger is CCIPReceiver, OwnerIsCreator {
    ...
    returns (bytes32 messageId)
  {
    ...
    Client.EVM2AnyMessage memory evm2AnyMessage = _buildCCIPMessage(_receiver, _payload, address(s_linkToken));
  ...
  {
    // Create an EVM2AnyMessage struct in memory with necessary information for sending a cross-chain message
    Client.EVM2AnyMessage memory evm2AnyMessage = _buildCCIPMessage(_receiver, _payload, address(0));
    ...
    // Emit an event with message details
    emit MessageSent(messageId, _destinationChainSelector, _receiver, _payload, address(0), fees);
